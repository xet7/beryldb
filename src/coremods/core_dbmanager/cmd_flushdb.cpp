...
{  
       user->SendProtocol(BRLD_QUERY_FLUSHING, Daemon::Format("OK: %s", Kernel->Store->Default->GetName().c_str()));
       ...
       user->SendProtocol(ERR_UNABLE_FLUSH, Daemon::Format("%s: %s.", PROCESS_FALSE, Kernel->Store->Default->GetName().c_str()));
       return FAILED;
}